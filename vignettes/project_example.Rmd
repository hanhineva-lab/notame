...
ionization mode) or whatever split was supplied to `import_from_excel()`.
...
for (i in seq_along(modes)) {
  ...
  save_QC_plots(corrected, prefix = paste0(ppath, "figures/", name, "_DRIFT"),
                perplexity = 5, group = "Group", time = "Time", 
                id = "Subject_ID", color = "Group", assay.type = "corrected")
  # Flag low-quality features
  corrected <- corrected |> 
    assess_quality(object = _, assay.type = "corrected") |> 
    flag_quality(object = _, assay.type = "corrected")
...
```{r}
merged <- merge_notame_sets(processed, assay.type = "corrected")
