...
which fall outside the purview of *notame*.

Inclusion of *notame* in Bioconductor and SummarizedExperiment-support promotes 
reproducible, end-to-end analyses. For example, the xcms package can be used 
for preprocessing, outputting a SummarizedExperiment object using 
xcms::quantify(). Then notame can be used for data pretreatment and feature 
selection with complementary functionality in the pmp, phenomis and qmtools 
packages. There are also SummarizedExperiment-supporting packages for 
downstream steps, such as MetaboAnnotation for annotation. 
...
preparation of the metadata to prepare the data for analysis. To 
...
Alternatively, data in R can be wrangled and passed to construct_metabosets or 
the native SummarizedExperiment constructor. 
...
using init_log(), log_text() and finish_log(). *notame* also keeps track of all 
the external packages used, offering you references for each. To see and log a 
list of references, use citations().
