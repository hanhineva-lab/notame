...
# How it works
SummarizedExperiment is the primary data structure of this package. One can use 
a single peak table throughout the analysis as with MetaboSet and also use 
multiple peak tables with SummarizedExperiment, using the `assay.type` and 
`name` arguments.
...
be returned as ggplot objects. The `notameViz::save_QC_plots()` wrapper can be 
used for saving central visualizations at different stages of processing.
