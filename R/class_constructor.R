...
.check_assay <- function(assay_, log_messages = FALSE) {
...
.extract_from_excel <- function(file, sheet, corner_row, corner_column, name) {
  ...
  log_text(paste0("\nExtracting feature abundances from rows ", cr + 1, " to ",
                  ...
                  excel_columns[ncol(dada)]))
  assay_ <- dada[(cr + 1):nrow(dada), (cc + 1):ncol(dada)]
...
.check_integrity <- function(
  ...
  .check_assay(assay_, log_messages = TRUE)
...
# Helper function to validate import arguments
.check_import_args <- function(skip_checks, split_by, name) {
  if (!skip_checks && is.null(split_by) && is.null(name)) {
    stop("Either name or split_by needs to be defined, see documentation.")
  } else if (!skip_checks && !is.null(split_by) && !is.null(name)) {
...
#' If set to NULL, will be detected automatically.
#' @param id_prefix character, prefix for autogenerated sample IDs
...
#'
#' @return A SummarizedExperiment with assays, rowData and colData filled from
#' the Excel file.
#'
#' @examples
...
import_from_excel <- function(file, sheet = 1, id_column = NULL, 
                          ...
                          skip_checks = FALSE) {
  .check_import_args(skip_checks, split_by, name)
  extracted <- .extract_from_excel(file, sheet, corner_row, 
                                    corner_column, name)
  # Skip checks
  if (!skip_checks) {
    extracted <- .check_integrity(extracted, id_prefix = id_prefix,
                     id_column = id_column, split_by = split_by, name = name,
...
.extract_from_tsv <- function(file, name) {
  ...
  sample_cols <- which(raw[1, ] != "" & !is.na(raw[1, ]))
  # first column tells us which rows hold features
  feature_rows <- which(raw[, 1] != "" & !is.na(raw[, 1]))
  # columns and rows overlap by one between feature/sample data
  feature_cols <- seq_len((min(sample_cols)))
  sample_rows <- seq_len((min(feature_rows)))

  # Gather feature data
  feature_data <- raw[feature_rows[-1], feature_cols, drop = FALSE]
  colnames(feature_data) <- raw[feature_rows[1], feature_cols]

  # Gather sample data
  sample_data <- as.data.frame(
    t(raw[sample_rows, sample_cols[-1]]),
    stringsAsFactors = FALSE
  )
  log_text(paste0("Replacing spaces in sample information column names with", 
           " underscores (_)"))
  colnames(sample_data) <- gsub(" ", "_", raw[sample_rows, sample_cols[1]])
  # If a single mode is given, datafile will indicate the mode
  if (!is.null(name)) {
    log_text(paste0('Naming the last column of sample information "', name,
                    '_Datafile"'))
...
#' similarly to \code{\link{import_from_excel}}, but removes the need to 
...
#'
#' @export
import_from_msdial <- function(file, id_column = NULL,
                          id_prefix = "ID_", split_by = NULL, name = NULL,
                          mz_limits = c(10, 2000), rt_limits = c(0, 20),
                          skip_checks = FALSE) {
  .check_import_args(skip_checks, split_by, name)
  extracted <- .extract_from_tsv(file, name)
  # Skip checks
  if (!skip_checks) {
    extracted <- .check_integrity(extracted, id_prefix = id_prefix,
                     id_column = id_column, split_by = split_by, name = name,
...
write_to_excel <- function(object, file, ...) {
  ...
  # Replace assay column names with the last column (now row) of sample info
...
.check_object <- function(object, pheno_injection = FALSE, pheno_ID = FALSE, 
  ...
  .check_assay(assay(object, assay.type))
  }
...
fix_object <- function(object, id_prefix = "ID_", id_column = NULL,
  ...
  colnames(assay) <- rownames(pheno_data)
  ...
  if (split_data) {
    ...
    for (part in parts) {
      ...
      ad_tmp <- assay[fd_tmp$Feature_ID, ]
